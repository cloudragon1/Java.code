...
<script>
import { listFuel, getFuel, delFuel, addFuel, updateFuel } from "@/api/fuel/fuel"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除动力类型编号为"' + ids + '"的数据项？').then(function() {
        return delFuel(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('fuel/fuel/export', {
        ...this.queryParams
      }, `fuel_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
